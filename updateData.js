...
const { checkDiagnoseGeneral, checkDiagnoseSkin, checkDiagnoseDentist, checkDiagnoseOther, checkDiagnoseEarNeckNose, checkDiagnoseEye, checkDiagnoseBone, checkDiagnoseBaby } = require('./fire_CSV/analysis')

module.exports = async (request, response) => {
    // import csvtojson
    const csvFilePath = './uploads/clinicCSV.csv'
        ...
        .then((jsonObj) => {
            for (let i = 0; i < jsonObj.length; i++) {
                ...
                if (jsonObj[i].Facebookpage) {
                    ...
                    if (checkDiagnoseOther(store, about, facebookpage, statuspage) == 'yes') {
                        let docRef = db.collection('otherClinic').doc(`${jsonObj[i].ชื่อสถานพยาบาล}`)
                        let setAda = docRef.set({
                            name: checkDataInput(jsonObj[i].ชื่อสถานพยาบาล),
                            information: {
                                เลขที่ใบอนุญาต: checkDataInput(jsonObj[i].เลขที่ใบอนุญาต),
                    ...
                    let setAda = docRef.set({
                        ...
                        diagnose: {
                            ...
                            other: checkDiagnoseOther(store, about, facebookpage, statuspage),
