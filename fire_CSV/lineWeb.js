...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  function generalClinic(agent) {
    let a =[];
	return admin.firestore().collection('generalClinic').orderBy("name").limit(5).get().then(snapshot => {
  ...
  function boneClinic(agent) {
    let a = [];
	return admin.firestore().collection('boneClinic').orderBy("name").limit(5).get().then(snapshot => {
      snapshot.forEach(doc => {
        a.push(doc.data());
        //agent.add(doc.data().name);
  ...
  function eyeClinic(agent) {
    let a = [];
	return admin.firestore().collection('eyeClinic').orderBy("name").limit(5).get().then(snapshot => {
      snapshot.forEach(doc => {
        a.push(doc.data());
        //agent.add(doc.data().name);
        //agent.add(doc.data().socialdata.about);
  ...
  function earNeckNoseClinic(agent) {
    let a = [];
	return admin.firestore().collection('earNackNoseClinic').orderBy("name").limit(5).get().then(snapshot => {
      snapshot.forEach(doc => {
        a.push(doc.data());
        //agent.add(doc.data().name);
        //agent.add(doc.data().socialdata.about);
  ...
  intentMap.set('Symptom - Ear - yes', earNeckNoseClinic);
